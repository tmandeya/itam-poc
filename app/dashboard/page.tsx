...
// ── KPI Card ──
function KPICard({ icon: Icon, label, value, subtext, color = '#FFD700' }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 28, fontWeight: 700, color: '#fff', letterSpacing: -0.5 }}>{value}</div>
...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    if (assets) {
      ...
      setStats({
        ...
        in_transit_assets: inTransit,
        disposed_assets: disposed,
        in_store_assets: inStore,
        total_purchase_value: totalPurchase,
        total_current_value: totalPurchase * 0.65, // approximate
  ...
  const pieColors = ['#4ADE80', '#60A5FA', '#FB923C', '#A78BFA', '#F87171'];
    ...
    if (active && payload?.length) {
      return (
        <div style={{
          ...
          <p style={{ color: '#FFD700', margin: '0 0 4px', fontWeight: 600 }}>{label}</p>
          {payload.map((p: any, i: number) => (
            <p key={i} style={{ color: p.color, margin: 2, fontSize: 11 }}>
              {p.name}: {p.value}
            </p>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="animate-fade-in">
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        ...
        <KPICard icon={DollarSign} label="Purchase Value" value={`$${((stats?.total_purchase_value || 0) / 1000).toFixed(0)}k`} subtext="Total acquisition cost" color="#4ADE80" />
        <KPICard icon={TrendingDown} label="Current Value" value={`$${((stats?.total_current_value || 0) / 1000).toFixed(0)}k`} subtext="After depreciation" color="#FB923C" />
        <KPICard icon={Wrench} label="In Repair" value={stats?.in_repair_assets || 0} color="#F87171" />
        <KPICard icon={AlertTriangle} label="Warranty Expiring" value={stats?.warranty_expiring_soon || 0} subtext="Within 90 days" color="#FB923C" />
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{
          flex: 2, minWidth: 400, background: '#141414', border: '1px solid #2A2A2A',
          borderRadius: 12, padding: 20,
        }}>
          <h4 style={{ margin: '0 0 16px', color: '#F0F0F0', fontSize: 15, fontWeight: 600 }}>
            Assets by Site
          </h4>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={siteData}>
              ...
              <Bar dataKey="assets" fill="#FFD700" radius={[4, 4, 0, 0]} name="Assets" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Status Pie */}
        <div style={{
          flex: 1, minWidth: 280, background: '#141414', border: '1px solid #2A2A2A',
          borderRadius: 12, padding: 20,
        }}>
          <h4 style={{ margin: '0 0 16px', color: '#F0F0F0', fontSize: 15, fontWeight: 600 }}>
            Status Breakdown
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
            {statusData.map((s, i) => (
              <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#999' }}>
                <span style={{ width: 8, height: 8, borderRadius: 2, background: pieColors[i] }} />
                {s.name}: {s.value}
              </div>
            ))}
          </div>
      ...
      {/* Bottom Row: Sites + Activity */}
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {/* Site Overview */}
        <div style={{
          flex: 1, minWidth: 350, background: '#141414', border: '1px solid #2A2A2A',
          borderRadius: 12, padding: 20,
        }}>
          <h4 style={{ margin: '0 0 16px', color: '#F0F0F0', fontSize: 15, fontWeight: 600 }}>
            Site Overview
            ...
            return (
              <div key={site.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#F0F0F0' }}>{site.id}</div>
                    <div style={{ fontSize: 10, color: '#666' }}>{site.city}, {site.country}</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#FFD700' }}>{count}</div>
                  <div style={{ fontSize: 9, color: '#666', textTransform: 'uppercase' }}>Assets</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{
          ...
          {recentActivity.length === 0 ? (
            <p style={{ color: '#666', fontSize: 13, textAlign: 'center', padding: 40 }}>
              No activity yet. Add your first asset to get started!
            </p>
          ) : (
            recentActivity.map((log, i) => (
              <div key={log.id} style={{
                display: 'flex', gap: 12, padding: '10px 0',
                borderBottom: i < recentActivity.length - 1 ? '1px solid #2A2A2A' : 'none',
              }}>
                <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, color: '#F0F0F0', textTransform: 'capitalize' }}>
