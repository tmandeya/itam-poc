...
export default function UsersPage() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* ── ADD USER MODAL ── */}
      {showAddModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 20px', overflowY: 'auto' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', border: '1px solid #E0E0E0', borderRadius: 16, width: '100%', maxWidth: 540, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            ...
            <form onSubmit={handleAddUser} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              {addError && (
                <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 4, textTransform: 'uppercase', fontWeight: 600 }}>Full Name *</label>
                  <div style={{ position: 'relative' }}>
                    <User size={15} color="#999" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)' }} />
                    <input required value={addForm.full_name}
                           onChange={e => setAddForm(f => ({...f, full_name: e.target.value}))}
                           placeholder="John Doe" style={{ ...inputStyle, paddingLeft: 34 }} />
                  </div>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 4, textTransform: 'uppercase', fontWeight: 600 }}>Employee ID</label>
                  <input value={addForm.employee_id}
                         onChange={e => setAddForm(f => ({...f, employee_id: e.target.value}))}
                         placeholder="EMP-001" style={inputStyle} />
                </div>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 4, textTransform: 'uppercase', fontWeight: 600 }}>Email *</label>
                <div style={{ position: 'relative' }}>
                  <Mail size={15} color="#999" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)' }} />
                  <input required type="email" value={addForm.email}
                         onChange={e => setAddForm(f => ({...f, email: e.target.value}))}
                         placeholder="user@magayamining.com" style={{ ...inputStyle, paddingLeft: 34 }} />
                </div>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 4, textTransform: 'uppercase', fontWeight: 600 }}>Temporary Password *</label>
                <div style={{ position: 'relative' }}>
                  <input required type={showPassword ? 'text' : 'password'} minLength={6}
                         value={addForm.password}
                         onChange={e => setAddForm(f => ({...f, password: e.target.value}))}
                         placeholder="Minimum 6 characters" style={{ ...inputStyle, paddingRight: 36 }} />
                ...
                <p style={{ fontSize: 10, color: '#999', margin: '4px 0 0' }}>User should change this after first login</p>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 6, textTransform: 'uppercase', fontWeight: 600 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, color: '#666', marginBottom: 6, textTransform: 'uppercase', fontWeight: 600 }}>
                  <Building2 size={12} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Assign Sites
                </label>
                  ...
                  {sites.map(site => (
                    <button key={site.id} type="button" onClick={() => toggleAddSite(site.id)} style={{
                      ...
                      <span style={{
                        ...
                        border: `1px solid ${addForm.site_ids.includes(site.id) ? '#D4A800' : '#E0E0E0'}`,
                        background: addForm.site_ids.includes(site.id) ? '#D4A800' : '#fff',
                      }}>
                        {addForm.site_ids.includes(site.id) && <Check size={10} color="#fff" />}
                      </span>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
                ...
                <button type="submit" disabled={addLoading} style={{
                  display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px',
                  borderRadius: 8, border: 'none', background: addLoading ? '#B8960C' : '#D4A800',
                  color: '#fff', fontSize: 13, fontWeight: 600,
                  cursor: addLoading ? 'not-allowed' : 'pointer',
                }}>
                  {addLoading ? <Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> : <Plus size={15} />}
      ...
      {editUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', border: '1px solid #E0E0E0', borderRadius: 16, width: '100%', maxWidth: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', borderBottom: '1px solid #E0E0E0' }}>
            ...
            <div style={{ padding: 24 }}>
                  ...
                  {sites.map(site => (
                    <button key={site.id} onClick={() => toggleSite(site.id)} style={{
                      ...
                      <span style={{
                        width: 16, height: 16, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center',
                        border: `1px solid ${editSites.includes(site.id) ? '#D4A800' : '#E0E0E0'}`,
                        background: editSites.includes(site.id) ? '#D4A800' : '#fff',
                      }}>
                        {editSites.includes(site.id) && <Check size={10} color="#fff" />}
                      </span>
                      <span><strong>{site.id}</strong> — {site.city}</span>
                    </button>
                  ))}
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => setEditUser(null)} style={{ padding: '9px 18px', borderRadius: 8, border: '1px solid #E0E0E0', background: '#fff', color: '#1A1A1A', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
