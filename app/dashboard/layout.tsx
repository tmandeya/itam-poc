...
interface AppContextType {
  profile: Profile | null; sites: Site[]; userSiteIds: string[];
  selectedSite: string; setSelectedSite: (s: string) => void;
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const { data: sitesData } = await supabase.from('sites').select('*').eq('is_active', true).order('id');
      if (sitesData) setSites(sitesData);
      if (profileData?.role === 'super_admin') {
        setUserSiteIds(sitesData?.map(s => s.id) || []);
      } else {
        const { data: assignments } = await supabase.from('user_site_assignments').select('site_id').eq('user_id', user.id);
    ...
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff' }}>
        <div style={{ textAlign: 'center' }}>
          <Image src="/images/logo.png" alt="Magaya Mining" width={100} height={72} style={{ objectFit: 'contain', marginBottom: 16 }} priority />
  ...
  return (
    <AppContext.Provider value={{ profile, sites, userSiteIds, selectedSite, setSelectedSite }}>
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#F5F5F5' }}>
        {/* SIDEBAR */}
        <aside style={{
          width: collapsed ? 64 : 240, background: '#FFFFFF',
          ...
          <div style={{
            ...
            justifyContent: collapsed ? 'center' : 'flex-start', gap: 10, minHeight: 68,
          }}>
            <Image src="/images/logo.png" alt="Magaya Mining"
                   width={collapsed ? 36 : 44} height={collapsed ? 26 : 32}
                   style={{ objectFit: 'contain', flexShrink: 0 }} priority />
            {!collapsed && (
              <div>
                <div style={{ color: '#1A1A1A', fontWeight: 700, fontSize: 14, letterSpacing: 0.5, lineHeight: 1.1 }}>MAGAYA MINING</div>
                <div style={{ color: '#999', fontSize: 9, letterSpacing: 1.5, textTransform: 'uppercase' }}>IT Asset Management</div>
              </div>
            )}
          </div>
            ...
            {navItems.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => router.push(item.id)} style={{
                  ...
                  background: active ? 'rgba(212,168,0,0.1)' : 'transparent',
                  color: active ? '#B8960C' : '#666',
          ...
          <button onClick={() => setCollapsed(!collapsed)} style={{
            ...
            borderTop: '1px solid #E0E0E0', color: '#999', cursor: 'pointer',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <header style={{
            height: 56, borderBottom: '1px solid #E0E0E0', padding: '0 24px',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            background: '#FFFFFF', flexShrink: 0,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#1A1A1A' }}>{pageTitle}</h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <Globe size={13} color="#999" />
                <select value={selectedSite} onChange={(e) => setSelectedSite(e.target.value)} style={{
                  padding: '6px 10px', background: '#F0F0F0', border: '1px solid #E0E0E0',
                  borderRadius: 6, color: selectedSite ? '#1A1A1A' : '#999', fontSize: 12, cursor: 'pointer',
                }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{
                  ...
                  background: 'linear-gradient(135deg, #D4A800, #B8960C)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 12, fontWeight: 700, color: '#fff',
                }}>{initials}</div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#1A1A1A' }}>{profile?.full_name}</div>
                  <div style={{ fontSize: 10, color: '#B8960C', textTransform: 'capitalize' }}>{profile?.role?.replace('_', ' ')}</div>
                </div>
              </div>
              <button onClick={handleSignOut} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#999', padding: 4 }} title="Sign out">
                <LogOut size={16} />
              </button>
            </div>
          </header>
