...
import {
  ...
  XCircle, Package, MapPin, ChevronRight,
} from 'lucide-react';

const STATUS_COLORS: Record<string, string> = {
  pending_approval: '#FB923C', approved: '#60A5FA', in_transit: '#A78BFA',
  completed: '#4ADE80', rejected: '#F87171', cancelled: '#666',
};
...
export default function TransfersPage() {
  ...
  async function loadTransfers() {
    ...
    if (selectedSite) {
      query = query.or(`from_site_id.eq.${selectedSite},to_site_id.eq.${selectedSite}`);
    }
  ...
  async function handleCreateTransfer(e: React.FormEvent) {
    ...
    setShowModal(false);
    setForm({ asset_id: '', to_site_id: '', reason: '' });
    loadTransfers();
    loadAssets();
  }
  ...
  async function handleUpdateStatus(id: number, newStatus: string, assetId: number, toSiteId: string) {
    ...
    if (newStatus === 'rejected') {
      await supabase.from('assets').update({ status: 'active' }).eq('id', assetId);
    }
        ...
        {groups.map(status => {
          ...
          return (
            <div key={status} style={{
              flex: 1, minWidth: 260, background: '#141414', border: '1px solid #2A2A2A',
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#F0F0F0', textTransform: 'capitalize' }}>
                    {status.replace(/_/g, ' ')}
                  </span>
                </div>
                <span style={{
              ...
              <div style={{ flex: 1, padding: 8, display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 500, overflowY: 'auto' }}>
                {loading ? (
                  <p style={{ color: '#666', fontSize: 12, textAlign: 'center', padding: 20 }}>Loading...</p>
                ) : items.length === 0 ? (
                  <p style={{ color: '#666', fontSize: 11, textAlign: 'center', padding: 20 }}>No transfers</p>
                ) : items.map(t => (
                  <div key={t.id} style={{
                    background: '#1A1A1A', border: '1px solid #2A2A2A', borderRadius: 8, padding: 12,
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      <span style={{ color: '#FFD700', fontSize: 12, fontWeight: 600 }}>{t.transfer_ref}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6, fontSize: 12, color: '#F0F0F0' }}>
                      <MapPin size={11} color="#FB923C" /> {t.from_site_id}
                      <ChevronRight size={11} color="#666" />
                      <MapPin size={11} color="#4ADE80" /> {t.to_site_id}
                    </div>
                    {t.reason && <p style={{ fontSize: 11, color: '#666', margin: '4px 0' }}>{t.reason}</p>}
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {status === 'pending_approval' && (
                        <>
                          <button onClick={() => handleUpdateStatus(t.id, 'approved', t.asset_id, t.to_site_id)} style={{ flex: 1, padding: '5px 8px', borderRadius: 6, border: 'none', background: '#4ADE8020', color: '#4ADE80', fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>Approve</button>
                          <button onClick={() => handleUpdateStatus(t.id, 'rejected', t.asset_id, t.to_site_id)} style={{ flex: 1, padding: '5px 8px', borderRadius: 6, border: 'none', background: '#F8717120', color: '#F87171', fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>Reject</button>
                        </>
                      )}
                      {status === 'approved' && (
                        <button onClick={() => handleUpdateStatus(t.id, 'in_transit', t.asset_id, t.to_site_id)} style={{ flex: 1, padding: '5px 8px', borderRadius: 6, border: 'none', background: '#A78BFA20', color: '#A78BFA', fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>Mark Shipped</button>
                      )}
                      {status === 'in_transit' && (
                        <button onClick={() => handleUpdateStatus(t.id, 'completed', t.asset_id, t.to_site_id)} style={{ flex: 1, padding: '5px 8px', borderRadius: 6, border: 'none', background: '#4ADE8020', color: '#4ADE80', fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>Confirm Receipt</button>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 16, width: '100%', maxWidth: 500 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', borderBottom: '1px solid #2A2A2A' }}>
              <h3 style={{ margin: 0, color: '#FFD700', fontSize: 20 }}>Initiate Transfer</h3>
            ...
            <form onSubmit={handleCreateTransfer} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, color: '#999', marginBottom: 4, textTransform: 'uppercase', fontWeight: 600 }}>Destination Site *</label>
                <select required value={form.to_site_id} onChange={e => setForm(f => ({...f, to_site_id: e.target.value}))} style={{ ...inputStyle, color: form.to_site_id ? '#fff' : '#666' }}>
              ...
              <div>
                ...
                <textarea value={form.reason} onChange={e => setForm(f => ({...f, reason: e.target.value}))} placeholder="Reason for transfer..." rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
              </div>
